...
md`# HW5 Strong baseline (2pt)`
)}

function _strong(FileAttachment){return(
FileAttachment("group6.json").json()
)}

function _strong1(d3,strong,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 2000
    ;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(strong);
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["group6.json", {url: new URL("../group6.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("strong")).define("strong", ["FileAttachment"], _strong);
  main.variable(observer("strong1")).define("strong1", ["d3","strong","drag","url_list","invalidation"], _strong1);
